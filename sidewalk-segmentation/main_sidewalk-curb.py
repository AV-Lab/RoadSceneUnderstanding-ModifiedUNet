...
train_loader = getDatasetLoader(
	...
	dir_mask    = [TRAIN_MASK_BOUNDARY_DIR, TRAIN_MASK_SIDEWALK_DIR],
...
validation_loader  = getDatasetLoader(
	...
	dir_mask    = [VAL_MASK_BOUNDARY_DIR, VAL_MASK_SIDEWALK_DIR],
...
#model, optimizer = loadParameters(model, optimizer, name= "sidewalk_curb_model")
...
for epoch in range(NUM_EPOCHS):
	...
	with torch.no_grad():
		...
		for data, target in looper:
			...
			num_correct_sidewalk = ((preds == 2) & (target == 2)).sum()
			# Assign class for each pixel

			num_pixels  += torch.numel(preds)
			num_correct += (preds == target).sum()

			print(f'sidewalk: {num_correct_sidewalk/(target == 2).sum() * 100:.2f}%, ')
